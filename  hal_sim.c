...
// UART — prints to terminal
void hal_uart_send(const char *msg) {
    printf("[UART] %s\n", msg);
    fflush(stdout);
}

// I2C — simulates TMP102 temperature sensor at address 0x48
int hal_i2c_read(uint8_t addr, uint8_t reg, uint8_t *data, int len) {
    if (addr == 0x48) {
        ...
        printf("[I2C]  addr=0x%02X reg=0x%02X temp=%.2f°C\n", addr, reg, temp);
        return 0;
    }
    return -1;
}

// SPI — simulates DRV8323 register write
...
// PWM — drives the simulated motor model
void hal_pwm_set_duty(float duty_percent) {
    motor_model_set_pwm(duty_percent);
    printf("[PWM]  duty=%.1f%%\n", duty_percent);
}

// Encoder — reads ticks from motor model
